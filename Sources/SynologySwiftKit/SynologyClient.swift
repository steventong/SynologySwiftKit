...
public final class SynologyClient {
    ...
    public lazy var audioStation: AudioStationApi = {
        AudioStationApi(apiClient: apiClient)
    }()
    ...
    public lazy var quickConnect: QuickConnectApi = {
        QuickConnectApi(deviceConnection: deviceConnection)
    }()

    /// DSM 信息 API
    /// DSM info API
    public lazy var dsmInfo: DsmInfoApi = {
        DsmInfoApi(apiClient: apiClient)
    }()

    /// 加密 API
    /// Encryption API
    public lazy var encryption: EncryptionApi = {
        EncryptionApi(apiClient: apiClient)
    }()
    ...
    public init() {
        ...
        // 设置延迟依赖以解决循环依赖
        self.apiClient.apiInfoProvider = apiInfo
    }
}
