...
extension HTTPClient {
    ...
    private func sendRequest<T: Decodable>(_ request: URLRequest) async throws -> T {
        ...
        // 记录请求日志
        NetworkLogger.logRequest(url: url, method: request.httpMethod ?? "GET", headers: request.allHTTPHeaderFields, body: request.httpBody)
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                throw SynologyError.http("invalid response")
            }
            ...
            guard httpResponse.statusCode >= 200 && httpResponse.statusCode < 300 else {
                throw SynologyError.http("http request failed, status code: \(httpResponse.statusCode)")
            }
            ...
            do {
                ...
                throw SynologyError.http("decoding json failed: \(error.localizedDescription)")
            }
